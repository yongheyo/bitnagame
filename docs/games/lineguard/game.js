(() => {
  ...
  const WAVE_DEFS = [
    { count: 4, hp: 40, dmg: 8, speed: 42, gap: 1400 },
    { count: 6, hp: 48, dmg: 10, speed: 46, gap: 1200 },
    { count: 8, hp: 56, dmg: 12, speed: 50, gap: 1000 },
    { count: 10, hp: 68, dmg: 14, speed: 54, gap: 900 },
    { count: 12, hp: 80, dmg: 16, speed: 58, gap: 800 },
  ];
  ...
  class LineGuardScene extends Phaser.Scene {
    ...
    create() {
      ...
      this.add
        .rectangle(W / 2, this.laneY, W * 0.92, 36, 0x2a3358)
        .setStrokeStyle(2, 0x3d4a78);
      ...
      this.elixirBar = this.add
        .rectangle(W / 2 - (W * 0.7) / 2, 18, 0, 8, 0x7ef0c3)
        .setOrigin(0, 0.5);
    ...
    syncHandUi() {
      cardEls.forEach((el, i) => {
        ...
        el.innerHTML =
          hero.name + '<br /><span class="cost">비용 ' + hero.cost + "</span>";
    ...
    tryDeploy(heroIdx, fromAuto) {
      ...
      if (this.elixir < hero.cost) return false;
    ...
    spawnEnemy() {
      ...
      const body = this.add.rectangle(this.spawnX, y, 22, 28, 0xff5d8f);
    ...
    update(time, delta) {
      ...
      if (this.elixir < ELIXIR_MAX) {
        this.elixir = Math.min(ELIXIR_MAX, this.elixir + ELIXIR_RATE * dt);
      }
      ...
      if (!this.betweenWaves && this.spawnLeft > 0) {
  ...
  const boot = () => {
    ...
    const avail = el.clientWidth || Math.min(window.innerWidth - 24, 720);
    const w = Math.max(320, Math.min(720, avail));
    const h = Math.max(200, Math.round(w * 0.42));
    el.style.width = w + "px";
